...
async function fetchCloudHistory() {
  if (!CLOUD_CONFIG.apiKey || !CLOUD_CONFIG.binId) {
    return JSON.parse(localStorage.getItem('promptquest_history') || '[]');
  }

  try {
    const response = await fetch(`${CLOUD_CONFIG.apiUrl}/b/${CLOUD_CONFIG.binId}/latest`, {
    ...
    localStorage.setItem('promptquest_history', JSON.stringify(cloudHistory));
    return cloudHistory;
  } catch (e) {
...
async function pushCloudHistory(history) {
  if (!CLOUD_CONFIG.apiKey || !CLOUD_CONFIG.binId) {
    localStorage.setItem('promptquest_history', JSON.stringify(history));
    return;
  }

  try {
    await fetch(`${CLOUD_CONFIG.apiUrl}/b/${CLOUD_CONFIG.binId}`, {
      method: 'PUT',
      headers: {
...
async function saveToHistory(name, score, total) {
  const history = await fetchCloudHistory();
  history.unshift({
    name: name,
    score: score,
    total: total,
  ...
  await pushCloudHistory(history);
}
...
async function clearHistory() {
  localStorage.removeItem('promptquest_history');
  if (CLOUD_CONFIG.apiKey && CLOUD_CONFIG.binId) {
...
async function renderSecretHistory() {
  const history = await getHistory();
  const tbody = document.getElementById('history-table-body');
  const totalGames = document.getElementById('history-total-games');
  const bestScore = document.getElementById('history-best-score');
  const avgScore = document.getElementById('history-avg-score');
  const syncStatus = document.getElementById('history-sync-status');

  if (syncStatus) {
    syncStatus.textContent = CLOUD_CONFIG.apiKey ? 'SINCRONIZADO' : 'LOCAL APENAS';
    syncStatus.className = CLOUD_CONFIG.apiKey ? 'sync-ok' : 'sync-off';
  }
  ...
  if (history.length === 0) {
    ...
    tbody.innerHTML = '<tr class="empty-row"><td colspan="4">Nenhum resultado registrado ainda.</td></tr>';
    return;
  }
  ...
  avgScore.textContent = `${Math.round(sumPct / history.length)}%`;

  tbody.innerHTML = history.map((item, i) => `
    <tr>
      <td class="rank-cell">#${i + 1}</td>
      <td>${item.name}</td>
      <td class="score-cell">${item.score}/${item.total}</td>
      <td class="date-cell">${item.date}</td>
    </tr>
  `).join('');
}

function openSecretHistory() {
  renderSecretHistory();
  showScreen('screen-history');
  stopBGM();
}

function closeSecretHistory() {
...
function renderResult() {
  ...
  document.getElementById('result-player').textContent = `JOGADOR: ${state.playerName}`;
  ...
  saveToHistory(state.playerName, state.score, total);
...
// ============================================================
// SECRET HISTORY ACCESS (Ctrl+Shift+H)
// ============================================================
...
function loadCloudConfig() {
  const apiKey = document.getElementById('cloud-api-key');
  const binId = document.getElementById('cloud-bin-id');

  if (CLOUD_CONFIG.apiKey) apiKey.value = CLOUD_CONFIG.apiKey;
  if (CLOUD_CONFIG.binId) binId.value = CLOUD_CONFIG.binId;
}
...
document.getElementById('btn-save-config').addEventListener('click', () => {
  ...
  if (apiKey && binId) {
    ...
    localStorage.setItem('pq_api_key', apiKey);
    localStorage.setItem('pq_bin_id', binId);
    playCorrect();
    alert('Configuração salva! A sincronização está ativa.');
  } else {
    alert('Preencha ambos os campos (API Key e Bin ID).');
  }
});
...
document.getElementById('btn-test-sync').addEventListener('click', async () => {
  ...
  if (!apiKey || !binId) {
    alert('Preencha ambos os campos primeiro.');
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      playCorrect();
      alert('Conexão OK! Sincronização funcionando.');
    } else {
      playWrong();
      alert('Erro na conexão. Verifique os dados.');
    }
  } catch (e) {
    playWrong();
...
// Load config when history opens
const originalOpenSecretHistory = openSecretHistory;
openSecretHistory = function() {
  loadCloudConfig();
  originalOpenSecretHistory();
};
